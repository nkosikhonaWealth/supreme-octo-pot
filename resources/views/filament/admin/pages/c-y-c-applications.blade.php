<x-filament-panels::page>
    ...
    @if($participant->CYC)
		<div class="p-6 bg-white rounded-lg shadow-md">
	    	<h2 class="text-2xl font-bold mb-4 text-center text-blue-700">Application Details</h2>
	        <div class="grid grid-cols-2 gap-4 mt-4">
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Top SDGs & Alignment to the Commonwealth: </span> {{ $participant->CYC->sdg_response ?? 'Not specified' }}
                </p>
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Biggest Challenge Facing Youth & Advocacy Approach: </span> {{ $participant->CYC->challenge_response ?? 'No Response Provided.' }}
                </p>
                <p class="text-blue-700 mt-4">
				    <span class="font-bold">Represented Youth At Local / National / International Level:</span>
				    {{ isset($participant->CYC->representation_experience) ? ($participant->CYC->representation_experience ? 'Yes' : 'No') : 'No Response Provided.' }}
				</p>
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Where And How Youth Represented: </span> {{ $participant->CYC->representation_details ?? 'No Response Provided.' }}
                </p>
	        </div>
	        <div>
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Leadership / Advocacy Experience: </span> {{ $participant->CYC->leadership_experience ?? 'No Response Provided.' }}
                </p>
            </div>
            <div>
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Why I Should Be Chosen: </span> {{ $participant->CYC->motivation ?? 'No Response Provided.' }}
                </p>
            </div>
            <div class="grid grid-cols-2 gap-4 mt-4">
                    @if($participant->CYC->supporting_documents)
                        <div class="carousel-container relative" data-type="certificate">
                            @foreach($participant->CYC->supporting_documents as $key => $upload)
                                @php
                    ...
                    @endif
                    @if($participant->CYC->cv_upload)
                        <div class="carousel-container relative" data-type="certificate">
                            @foreach($participant->CYC->cv_upload as $key => $upload)
                                @php
                                    $extension = strtolower(pathinfo($upload, PATHINFO_EXTENSION));
                                    $isImage = in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp']);
                                @endphp
