<x-filament-panels::page>
    ...
    @if($participant->TOT)
		<div class="p-6 bg-white rounded-lg shadow-md">
	        ...
	        <div class="grid grid-cols-2 gap-4 mt-4">
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Employment Status: </span> {{ $participant->TOT->current_activity }}
                </p>
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Currently Resides In Inkhundla: </span> {{ $participant->TOT->current_residence }}
                </p>
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Ever Worked With Youth Organization: </span> {{ $participant->TOT->youth_organization_response }}
                </p>
                <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Youth Organization Name: </span> {{ $participant->TOT->youth_organization_name }}
                </p>
	        </div>
	        <div>
               <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Duties Within The Youth Organization: </span> {{ $participant->TOT->youth_organization_duties }}
                </p> 
            </div>
            <div>
               <p class="text-blue-700 mt-4">
                    <span class="font-bold"> Motivation To Apply To The Program: </span> {{ $participant->TOT->motivation }}
                </p> 
            </div>
            <div class="grid grid-cols-2 gap-4 mt-4">
                    @if($participant->TOT->certificate_upload)
                        <div class="carousel-container relative" data-type="certificate">
                            @foreach($participant->TOT->certificate_upload as $key => $upload)
                                @php
                    ...
                    @endif
                    @if($participant->TOT->cv_upload)
                        <div class="carousel-container relative" data-type="certificate">
                            @foreach($participant->TOT->cv_upload as $key => $upload)
                                @php
                                    $extension = strtolower(pathinfo($upload, PATHINFO_EXTENSION));
                                    $isImage = in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp']);
                                @endphp
